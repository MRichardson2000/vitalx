...
def test_fail_get_total_steps(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_total_steps()
    assert "Failed to retrieve total steps" in str(exc_info.value)
...
def test_fail_get_total_sleep(mock_fetch):
    mock_fetch.return_value = ()
    with pytest.raises(DatabaseError) as exc_info:
        get_total_sleep_time()
    assert "Failed to get total sleep time" in str(exc_info.value)
...
def test_fail_get_total_days_walked(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_total_days_walked()
    assert "Failed to get total days walked" in str(exc_info.value)
...
def test_fail_get_total_calories_burnt(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_total_calories_burnt()
    assert "Failed to get total calories burnt" in str(exc_info.value)
...
def test_fail_get_total_miles_walked(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_total_miles_walked()
    assert "Failed to get total miles walked" in str(exc_info.value)
...
def test_fail_get_favourite_walk_location(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_favourite_walk_location()
    assert "Failed to get favourite walk location" in str(exc_info.value)
...
def test_fail_get_total_days_slept(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_total_days_slept()
    assert "Failed to get total days slept" in str(exc_info.value)
...
def test_fail_get_walk_history(mock_fetch):
    mock_fetch.return_value = []
    result = get_walk_history()
    assert result == []
...
def test_fail_get_last_walk_date(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_last_walk_date()
    assert "Failed to get last walk date" in str(exc_info.value)
...
def test_fail_get_sleep_history(mock_fetch):
    mock_fetch.return_value = []
    result = get_sleep_history()
    assert result == []
...
def test_fail_get_latest_streak_entry(mock_fetch):
    mock_fetch.return_value = []
    with pytest.raises(DatabaseError) as exc_info:
        get_latest_streak_entry()
    assert "Failed to get latest streak due to" in str(exc_info.value)
...
@patch("vitalx.service.get_latest_streak_entry")
def test_fail_get_latest_streak(mock_get_streak_entry):
    mock_get_streak_entry.return_value = []
    result = get_latest_streak()
    assert result == 1
