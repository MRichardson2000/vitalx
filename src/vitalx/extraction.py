...
def export_tables_to_spreadsheets() -> None:
    ...
    try:
        ...
        for prefix, (folder_path, file_name) in TABLE_SQL_MAPPING.items():
            ...
            sql = load_sql_as_text(folder_path, file_name)
            ...
            if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
                logger.error(f"{output_path} either doesn't exist or the filesize is 0")
                raise CsvExportError(
                    f"Verification failed: {filename} is missing or empty."
                )
    ...
    for file_path in csv_files:
        ...
        if file_age > retention_sec:
            try:
                ...
                raise OSError(f"Unable to remove the file due to: {e}")
